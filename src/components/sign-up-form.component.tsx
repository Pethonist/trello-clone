...
export function SignUpForm() {
  ...
  return (
    <form onSubmit={onSubmit} className='block-wrapper'>
      <h1 className='text-3xl text-white font-bold text-center'>Register a new account</h1>
      ...
      {errors.root?.message && (
        <p className='text-center text-red-500 font-medium'>{errors.root.message}</p>
      )}
      <Input
        {...register('username')}
      ...
      />
      <Button isLoading={isSubmitting} disabled={!isValid || !isDirty} type='submit'>
        Sign up
      </Button>
    </form>
  );
